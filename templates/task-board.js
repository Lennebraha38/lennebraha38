...
async function createEkip() {
  ...
  if (!session) { showToast('Once giris yapmalisin!'); return; }
  ...
  if (error) { console.error(error.message); showToast('Ekip olusturulamadi!'); return; }
...
async function joinEkip(ekipId) {
  const session = (await supabase.auth.getSession()).data.session;
  if (!session) { showToast('Once giris yap!'); return; }
  ...
  if (error) {
    ...
    else { showToast(' Katilim basarisiz!'); }
    return;
  }
...
async function createGorev() {
  ...
  if (!baslik) { showToast(' Gorev basligi zorunlu!'); return; }
  ...
  if (error) { console.error(error.message); showToast(' Gorev olusturulamadi!'); return; }
