...
// Quantro UI Utilities
    ...
    (function(){
      ...
      // Tek yumusak isik sprite'i (her karede gradyan yok)
      ...
      const GLOW = makeGlow(20, 'rgba(175,205,255,0.38)');
      const CORES = ['210,225,255', '160,200,255', '140,220,255'];

      function resize(){
        W = canvas.width  = Math.ceil(window.innerWidth  * SCALE);
        H = canvas.height = Math.ceil(window.innerHeight * SCALE);
        ctx.setTransform(SCALE, 0, 0, SCALE, 0, 0);
        N = Math.max(10, Math.min(48, Math.round((window.innerWidth * window.innerHeight) / 20000)));
      ...
      function Particle(){
        ...
        this.r  = .4 + depth * .9;
        this.vx = (Math.random() - .5) * (.06 + depth * .12);
        this.vy = (Math.random() - .5) * (.06 + depth * .12);
        this.a  = .16 + depth * .34;
        this.si = (Math.random() * 3) | 0;
        this.tw = .0008 + Math.random() * .004;
      ...
      function draw(){
        ...
        for(let i = 0; i < N; i++){
          const p = particles[i];
          const tw = .7 + .3 * Math.sin(t * p.tw + p.tp);
