...
async function loadProfiller() {
  ...
  try {
    ...
    benEposta = s?.user?.email || '';
    const userIds = data.map(p => p.user_id).filter(Boolean);
    if (userIds.length) {
      const { data: kpuan } = await supabase.from('kpuanlar').select('kullanici_id, tarih').in('kullanici_id', userIds).order('tarih', { ascending: false });
      if (kpuan) kpuan.forEach(k => { if (!aktiviteMap[k.kullanici_id]) aktiviteMap[k.kullanici_id] = k.tarih; });
    }
  } catch (e) {}
  data.forEach(p => { grid.appendChild(createProfilCardElement(p, aktiviteMap[p.user_id], benEposta)); });
}

function createProfilCardElement(p, sonAktif, benEposta) {
  ...
  let durum = 'Çevrimdışı', durumRenk = '#6b7280';
  if (sonAktif) {
    const fark = (Date.now() - new Date(sonAktif).getTime()) / 1000 / 3600;
    if (fark < 1) { durum = 'Aktif'; durumRenk = '#10b981'; }
    else if (fark < 24) { durum = 'Son 24 saat'; durumRenk = '#22c55e'; }
    else if (fark < 168) { durum = 'Bu hafta'; durumRenk = '#eab308'; }
  } else if (p.musaitlik) { durum = p.musaitlik; durumRenk = '#7c3aed'; }
  ...
  card.innerHTML = (p.avatar_url
      ? '<div class="profil-avatar"><img src="'+p.avatar_url+'" alt="'+fullName+'" style="width:100%;height:100%;object-fit:cover;border-radius:16px;display:block;"></div>'
    ...
    '<div class="profil-available" style="--dot-color:'+durumRenk+'"><span style="background:'+durumRenk+'"></span>'+durum+'</div>'+
    ariyorRozet+
    ...
    (p.eposta && p.eposta !== benEposta ? '<button class="btn btn-ghost btn-sm" onclick="openDmModal(\''+p.eposta+'\', \''+fullName.replace(/'/g, '\u2019')+'\')" title="Mesaj g\u00f6nder">Mesaj</button>' : '')+
...
async function supabaseCreateProfil(fd, avatarFile) {
  ...
  if (fd.ariyorum) insertData.ariyorum = fd.ariyorum;
  const { data, error } = await supabase.from('profiller').insert(insertData).select().single();
...
async function supabaseUpdateProfil(id, fd, avatarFile) {
  ...
  if (fd.ariyorum) updateData.ariyorum = fd.ariyorum;
  const { error } = await supabase.from('profiller').update(updateData).eq('id', id);
...
function overrideProfilFunctions() {
  window.submitProfilForm = async function() {
    ...
    if (r) {
      ...
      if (!editId) { xpEkle('profil-olustur', 50); rozetVer('profil-tam'); }
      await loadProfiller();
...
// ===== TEMA =====
// Sadece dark mod — tema degistirme butonlari kaldirildi
localStorage.removeItem('quantro-theme');
...
window.openDmModal = function(aliciEmail, aliciAd) {
  ...
  dmTab(aliciEmail ? 'yeni' : 'gelen');
  m.classList.add('open');
  if (!aliciEmail) dmGelenKutusuYukle();
};
...
window.gonderDm = async function() {
  ...
  if (await dmGonder(alici, icerik)) { document.getElementById('dm-icerik').value = ''; dmTab('gelen'); dmGelenKutusuYukle(); }
};
window.dmTab = function(sekme) {
  const gelen = document.getElementById('dm-gelen-kutusu');
  const yeni = document.getElementById('dm-yeni-form');
  const tg = document.getElementById('dm-tab-gelen');
  const ty = document.getElementById('dm-tab-yeni');
  if (!gelen || !yeni) return;
  if (sekme === 'gelen') {
    gelen.style.display = ''; yeni.style.display = 'none';
    tg.classList.add('active'); ty.classList.remove('active');
    dmGelenKutusuYukle();
  } else {
    gelen.style.display = 'none'; yeni.style.display = '';
    tg.classList.remove('active'); ty.classList.add('active');
  }
};
async function dmGelenKutusuYukle() {
  const kutucuk = document.getElementById('dm-gelen-kutusu');
  if (!kutucuk) return;
  try {
    const s = (await supabase.auth.getSession()).data.session;
    if (!s) { kutucuk.innerHTML = '<div class="forum-bos">Mesajları görmek için giriş yapmalısın.</div>'; return; }
    const { data } = await supabase.from('dm_mesajlari').select('*').eq('alici_email', s.user.email).order('tarih', { ascending: false }).limit(50);
    if (!data || !data.length) { kutucuk.innerHTML = '<div class="forum-bos">Gelen kutusu boş. Profillerden birine "Mesaj" gönderebilirsin.</div>'; return; }
    kutucuk.innerHTML = data.map(m => `
      <div class="dm-mesaj ${m.okundu ? '' : 'yeni'}">
        <div class="dm-mesaj-ust"><b>${m.okundu ? '👁️' : '🆕'} ${m.gonderen_id.slice(0, 8)}</b><span>${new Date(m.tarih).toLocaleString('tr-TR')}</span></div>
        <div class="dm-mesaj-icerik">${m.icerik.replace(/</g, '&lt;').replace(/\n/g, '<br>')}</div>
      </div>`).join('');
    const ids = data.filter(m => !m.okundu).map(m => m.id);
    if (ids.length) { try { await supabase.from('dm_mesajlari').update({ okundu: true }).in('id', ids); } catch (e) {} }
    const rozet = document.getElementById('dm-bildirim-rozet');
    if (rozet) { rozet.textContent = ''; rozet.style.display = 'none'; }
  } catch (e) { kutucuk.innerHTML = '<div class="forum-bos">Gelen kutusu yüklenemedi.</div>'; }
}
      ...
      <div class="mentor-kart">
        ...
        <div class="mentor-bilgi">
          ...
          <div class="mentor-kart-alt">
            ${m.kullanici_id !== benId ? `<button class="btn btn-primary" onclick="mentorTalebiModal(${m.id}, '${m.ad.replace(/'/g, "\\'")}')">Mentorluk İste</button>` : '<span style="font-size:.78rem;color:var(--text-dim);">Bu senin mentor profilin</span>'}
          </div>
        </div>
      </div>`).join('');
  } catch (e) { console.error(e.message); }
...
async function ilanBasvurularim() {
  try {
    const s = (await supabase.auth.getSession()).data.session;
    if (!s) return;
    const { data, error } = await supabase.from('ilan_basvurulari').select('*, ilanlar(ad, iletisim_email)').eq('kullanici_email', s.user.email).order('tarih', { ascending: false });
    if (error) throw error;
...
async function initApp() {
  ...
  setupRealtime();
  // Veritabani (Supabase) suremleri tum sayfalarda etkin — formlar her yerden calissin
      ...
      overlay.innerHTML = `
        <div class="ilan-detail-modal">
          <div class="idm-header">
            ...
            <button class="idm-close-btn" onclick="closeIlanDetail()" aria-label="Kapat"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
          </div>
          <div class="idm-body">
            <div>
...
(function(){
      ...
      let lastPaint = 0;
      let t = 0;
      // Guclu cihazlarda 50fps'te boya, zayif cihazlarda 24fps — 360Hz hissi icin
      // yuksek tekrar, jank icin dusuk tekrar
      const FRAME_MS = (navigator.hardwareConcurrency || 4) >= 6 ? 20 : 41;
      ...
      function makeGlow(size, color){
        ...
        g.addColorStop(.4, color.replace(/[\d.]+\)$/, '.10)'));
        g.addColorStop(1, 'rgba(0,0,0,0)');
        c.fillStyle = g;
        c.fillRect(0, 0, size, size);
        return s;
      }
      const GLOW = makeGlow(26, 'rgba(190,215,255,0.42)');
      const CORES = ['235,242,255', '190,215,255', '150,195,255'];
      const SPARKLE = (Math.random() < .25) ? makeGlow(14, 'rgba(255,255,255,0.55)') : null;

      function resize(){
        W = canvas.width  = Math.ceil(window.innerWidth  * DPR);
        H = canvas.height = Math.ceil(window.innerHeight * DPR);
        ctx.setTransform(DPR, 0, 0, DPR, 0, 0);
        N = Math.max(12, Math.min(60, Math.round((window.innerWidth * window.innerHeight) / 16000)));
      ...
      function Particle(){
        ...
        this.r  = .55 + depth * 1.35;
        this.vx = (Math.random() - .5) * (.03 + depth * .08);
        this.vy = (Math.random() - .5) * (.03 + depth * .08);
        this.a  = .28 + depth * .44;
        this.si = (Math.random() * 3) | 0;
        this.tw = .0009 + Math.random() * .004;
        this.tp = Math.random() * Math.PI * 2;
        this.sw = .008 + Math.random() * .012;
        this.sp = Math.random() * Math.PI * 2;
        this.hero = depth > .72 && Math.random() < .5;
        this.sl = 2 + depth * 3;
      }
      ...
      function draw(){
        ...
        for(let i = 0; i < N; i++){
          const p = particles[i];
          const tw = .75 + .25 * Math.sin(t * p.tw + p.tp);
          ...
          // UHD parlak yildizlar: cekirdek + capraz isima
          if (p.hero && alpha > .45) {
            ctx.strokeStyle = `rgba(${CORES[p.si]},${alpha * .6})`;
            ctx.lineWidth = .6;
            const L = p.sl * (1 + .15 * Math.sin(t * p.tw * 2 + p.tp));
            ctx.beginPath();
            ...
            ctx.fillStyle = `rgba(${CORES[p.si]},${alpha})`;
            ctx.fill();
          }

          p.x += p.vx;
          p.y += p.vy + Math.sin(t * p.sw + p.sp) * .06;
          const M = 40;
          if (p.x < -M) p.x = window.innerWidth + M; else if (p.x > window.innerWidth + M) p.x = -M;
          if (p.y < -M) p.y = window.innerHeight + M; else if (p.y > window.innerHeight + M) p.y = -M;
        }
      }
    ...
    (function(){
      ...
      function render(){
        ...
        g.addColorStop(0,'rgba(59,130,246,.08)'); g.addColorStop(1,'transparent');
      ...
      let offscreen = false;
      let loopActive = true;
      function kickLoop(){ if (!loopActive) { loopActive = true; requestAnimationFrame(loop); } }
      if ('IntersectionObserver' in window) {
        new IntersectionObserver(entries => {
          offscreen = !entries[0].isIntersecting;
          if (!offscreen) kickLoop();
        }).observe(canvas);
      }
      document.addEventListener('visibilitychange', () => { if (!document.hidden) kickLoop(); });
      function loop(ts){
        if (document.hidden || offscreen) { loopActive = false; return; }
        if (ts - lastFrame >= 20) {
          lastFrame = ts;
          rotY += .0038 + mx*.0009;
    ...
    (function(){
      ...
      const RING_SPEED = 24;   // 360Hz hissi: hizli ve akici takip
      const DOT_SPEED  = 64;
      ...
      document.addEventListener('mousemove', e => { mx = e.clientX; my = e.clientY; lastMove = performance.now(); if (!running) { running = true; requestAnimationFrame(loop); } }, { passive: true });
      ...
      function loop(ts) {
        // Fare 300ms'den uzun sure hareketsizse donguyu tamamen kapat
        if (ts - lastMove > 300) { running = false; return; }
        ...
        // Yerlesme toleransina ulasildiysa stil yazmayi birak (kalem cekilmis gibi donuyor)
        const settled = Math.abs(dx - mx) < .05 && Math.abs(dy - my) < .05 && Math.abs(rx - mx) < .3 && Math.abs(ry - my) < .3;
        if (!settled || hoverDirty) {
    ...
    function editProfilCard(btn) {
      ...
      setTimeout(() => {
        ...
        // Tum alanlari doldur — aksi halde Guncelle, zorunlu alan bos diye sessizce iptal olur
        if (card.dataset.iletisim) document.getElementById('pf-iletisim').value = card.dataset.iletisim;
        const sehirEl = document.getElementById('pf-sehir');
        if (sehirEl && card.dataset.sehir) sehirEl.value = card.dataset.sehir;
        const saatEl = document.getElementById('pf-saat');
        ...
        document.getElementById('pf-ariyorum').value = card.dataset.ariyorum || '';
        ...
        if (submitBtn) {
          ...
          submitBtn.onclick = async function() {
            ...
            // Supabase yolu (sayfa override'ini kullanir) — veritabaninda gunceller
            const w = window.submitProfilForm;
            if (typeof w === 'function' && w !== submitProfilForm) {
              await w();
            } else {
              submitProfilForm();
            }
            if (oldCard && oldCard.parentNode) oldCard.remove();
            const closed = !document.getElementById('profil-form-overlay').classList.contains('open');
            if (closed) {
    ...
    function openProfilDetail(card) {
      ...
      const ariyorum = card.dataset.ariyorum || '';
      const eposta   = card.dataset.eposta || '';
      const alanLabel= {yazilim:'Yazılım',  'yapay-zeka':'Yapay Zeka', donanim:'Donanım / Elektronik', tasarım:'Tasarım', siber:'Siber Güvenlik', mekanik:'Mekanik'}[alan] || alan;
      const durumBadge = card.querySelector('.profil-available span');
      const durum = card.querySelector('.profil-available')?.textContent?.trim() || 'Çevrimdışı';
      const durumRenk = durumBadge?.style?.background || '#6b7280';
      ...
      overlay.innerHTML = `
        <div class="profil-detail-modal">
          <div class="pdm-cover">
            <button class="pdm-close-btn" onclick="closeProfilDetail()" aria-label="Kapat"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
          ...
          <div class="pdm-body">
            <div class="pdm-header">
              ...
              <div class="pdm-available-badge" style="background:${durumRenk}22;color:${durumRenk};border:1px solid ${durumRenk}44">${durum}</div>
            </div>
            <div class="pdm-bio">${escapeHtml(bio) || '<span style="color:var(--text-dim);font-style:italic;">Biyografi eklenmemiş.</span>'}</div>
            ${ariyorum ? `<div class="profil-ariyor" style="margin-top:.7rem;"><span>🧲</span>${escapeHtml(ariyorum)}</div>` : ''}
            ${skills.length ? `
            ...
            ${eposta ? `<button class="idm-apply-btn" style="background:var(--primary);color:#fff;border:none;" onclick="closeProfilDetail();openDmModal('${escapeHtml(eposta)}', '${escapeHtml(name)}')">
    ...
    function toggleParticleSetting(btn) {
      ...
      const on = btn.classList.contains('on');
      const canvas = document.getElementById('v-bg-canvas');
      if (canvas) canvas.style.opacity = on ? '.8' : '0';
    }
    ...
    function toggleAyar(id) {
      ...
      if (id === 'cursor-toggle' || id === 'particle-toggle') {
        ...
        if (id === 'particle-toggle' && canvas) canvas.style.opacity = on ? '.8' : '0';
      }
    }
