...
// Quantro UI Utilities
    ...
    (function(){
      ...
      // Glow sprite'lari tek sefer cizilir (her karede gradyan maliyeti yok)
      function makeSprite(size, color){
        ...
        g.addColorStop(.55, color.replace(/[\d.]+\)$/, '.12)'));
      ...
      const SPRITES = [
        makeSprite(26, 'rgba(59,130,246,0.34)'),
        makeSprite(30, 'rgba(6,182,212,0.32)'),
        makeSprite(26, 'rgba(139,92,246,0.30)')
      ];
      const CORES = ['59,130,246', '6,182,212', '139,92,246'];

      function resize(){
        W = canvas.width  = Math.ceil(window.innerWidth  * DPR);
        H = canvas.height = Math.ceil(window.innerHeight * DPR);
        ctx.setTransform(DPR, 0, 0, DPR, 0, 0);
        N = Math.max(10, Math.min(52, Math.round((window.innerWidth * window.innerHeight) / 18000)));
        particles = Array.from({length: N}, () => new Particle());
      }

      function Particle(){
        const depth = Math.random();
        this.x  = Math.random() * (W / DPR || 100);
        this.y  = Math.random() * (H / DPR || 100);
        this.r  = 1 + depth * 1.6;
        this.vx = (Math.random() - .5) * (.14 + depth * .3);
        this.vy = (Math.random() - .5) * (.14 + depth * .3);
        this.a  = .25 + depth * .42;
        this.si = (Math.random() * 3) | 0;
        this.tw = .0012 + Math.random() * .003;
      ...
      function draw(){
        ...
        for(let i = 0; i < N; i++){
          ...
          // Fare itmesi (yumusak tepme)
          const mdx = p.x - mouse.x, mdy = p.y - mouse.y;
          const md2 = mdx*mdx + mdy*mdy;
          if (md2 < MOUSE2 && md2 > .01){
          ...
          // Baglanti cizgileri (sqrt'suz)
          for(let j = i+1; j < N; j++){
            const q  = particles[j];
            const dx = p.x-q.x, dy = p.y-q.y;
            const d2 = dx*dx+dy*dy;
            if(d2 < LINK2){
              ctx.strokeStyle = `rgba(120,160,255,${(1-d2/LINK2)*.14})`;
              ctx.lineWidth   = 1;
              ctx.beginPath(); ctx.moveTo(p.x,p.y); ctx.lineTo(q.x,q.y); ctx.stroke();
            }
          }

          // Fare-yakinlik cizgisi
          if(md2 < 160*160){
            ctx.strokeStyle = `rgba(6,182,212,${(1-md2/(160*160))*.3})`;
            ctx.lineWidth   = .8;
            ctx.beginPath(); ctx.moveTo(p.x,p.y); ctx.lineTo(mouse.x,mouse.y); ctx.stroke();
          }

          // Parildama (twinkle) ile glow + cekirdek
          const tw   = .72 + .28 * Math.sin(t * p.tw + p.tp);
          const alpha = Math.min(1, p.a * tw);
          const spr = SPRITES[p.si];
          ...
          // Hareket (kenarda tasip geri gel)
          p.x += p.vx; p.y += p.vy;
          const M = 40;
          if (p.x < -M) p.x = W/DPR + M; else if (p.x > W/DPR + M) p.x = -M;
          if (p.y < -M) p.y = H/DPR + M; else if (p.y > H/DPR + M) p.y = -M;
        }
      }
